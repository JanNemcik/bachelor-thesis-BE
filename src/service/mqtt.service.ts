...
export class MqttService {
		...
		pipe(
			mergeMap((doc: LogModel) =>
				obs1.pipe(
					timeout(3000),
					catchError(err => {
						console.log('1vy sync', doc, err.message);
						this.mqttProvider.repeatSyncProcess(doc.topic, doc.hash);
						return throwError({ err: err.message, doc });
					}),
					retry(2),
					take(1),
					mapTo(doc)
			...
			catchError((errorManager: MqttLogManager) => {
				...
				return this.updateLog(errorManager, LogStateEnum.ERRORED).pipe(
					tap(({ err, topic }) =>
	 ...
	 * @memberof MqttService
	 */
	private updateLog(log: any, state: LogStateEnum) {
		console.log('update log', log);
		log.doc.state = state;
		log.doc.err = log.err;
		return from(log.doc.save());
	}
}
